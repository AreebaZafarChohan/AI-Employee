...
VALID_ITEM_TYPES = {"email", "file_drop", "whatsapp", "linkedin_post"}
...
def execute_publish_post_action(plan_metadata: dict, approval_metadata: dict) -> dict:
    """Execute publish post action via MCP LinkedIn server.
    ...
    logger.info(f"Executing publish post action for plan {plan_id}")

    # Load post content from plan or source
    post_content = plan_metadata.get("post_content", "")
    if not post_content:
    ...
    result = call_mcp_tool("publish_linkedin_post", mcp_params)
...
def run_orchestrator_cycle() -> int:
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_EXECUTIONS) as executor:
        future_to_file = {
            executor.submit(process_approval_file, path): path
